...
Both payment integrations feed into a unified schema in `convex/schema.ts`. The exact status and remaining time of a user's subscription is continuously tracked and synced.
...
users: defineTable({
  ...
  currentPeriodEnd:     v.optional(v.number()),  // Unix timestamp in ms dictating EXACT cycle end
